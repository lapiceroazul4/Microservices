...
async function getAirbnbById(id) {
  try {
    const [rows] = await pool.query("SELECT * FROM micro_airbnbs WHERE id = ?", id);
    return rows[0];
  } catch (error) {
...
async function getAirbnbByHostId(hostId) {
  try {
    const [rows] = await pool.query("SELECT * FROM micro_airbnbs WHERE host_id = ?", [hostId]);
    return rows;
  } catch (error) {
...
async function getAirbnbsByRoomType(room_type) {
    try {
      const [rows] = await pool.query("SELECT * FROM micro_airbnbs WHERE room_type = ?", [room_type]);
      return rows;
    } catch (error) {
...
async function updateAirbnb(id, airbnbData) {
  try {
    const result = await pool.query("UPDATE micro_airbnbs SET ? WHERE id = ?", [airbnbData, id]);
    let message
...
async function deleteAirbnbById(id) {
  try {
    let message
    const result = await pool.query("DELETE FROM micro_airbnbs WHERE id = ?", id);
